...
<body>
    ...
    if($result = $mysqli->query('SELECT * FROM user ')) {   // $mysqli - наш об'єкт, через який здійснюємо підключення, query - метод, який дозволяє виконати довільний запит
        ...
        printf("Users: <br><br>");   // <br> в html - розрив рядка
        printf("<table><tr><th>ID</th><th>Nickname</th><th>Rating</tr>");   // <br> в html - розрив рядка
        ...
        while( $row = $result->fetch_assoc() ){ // fetch_assoc() повертає рядок із запиту у вигляді асоціативного масиву, наприклад $row = ['id'=>'1', 'pib'=>'Олександр', 'grupa'=>'ІПЗ-31']
            printf("<tr><td>%s</td><td>%s</td><td>%s</td></tr>", $row['ID'], $row['Nickname'], $row['Rating']); //виводимо результат на сторінку
        };
        printf("</table>");
        /*Звільняємо пам'ять*/
        $result->close();
    ...
    <ul>
        ...
        <li><a href="updateUsersForm.php">Змінити рядок</a><br></li>
        <li><a href="deleteFromUsersForm.php">Видалити рядок</a><br></li>
        <li><a href="showUsersZvit.php">Zvit - Nickname / Community/ Password</a><br></li>
